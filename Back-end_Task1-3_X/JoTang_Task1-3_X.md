...
**后期笔记可能有一些没有同步到这里，放提交文件夹里面去了，以提交为准**
有很多记录都直接注释到对应的代码旁边了，感觉比较直观。

### Github地址 https://github.com/LKC2006/JoTang2025
### 同时提供了从Docker Hub拉取并运行的脚本，需要执行权限
### 不如直接访问http://47.109.204.208:8080吧

### 写在前面
在入门了一下嵌入式后，我其实犹豫了一下要不要开始做这个后端的项目，毕竟已经花掉了那么长时间，同时还是零基础,开学的时候VScode都配置了一天。但是因为还是有兴趣，所以最终决定抛开那些想法，开始入门一下后端，最后也有了一定的完成度。这个项目也是根据上线/本地调试/推到Docker需要稍微更改一下配置文件。
...
Maven的项目一开始有被繁杂的目录和层叠的文件惊讶到，结果发现其实做项目的时候真正在写的就是一小部分，也可以做的非常有条理，非可有明确的分工。dao,controller,service之类的分层  
...
lombok的依赖连同它的各种方法（log等）全部报错，更新版本等方法都没有用。实在是被卡住了，很多方法都尝试过了也解决不了，于是我就选择把这个全部注释掉了，也只能不输出日志，补全了getter/setter方法和构造方法。   
然后就可以打包为jar包了，第一次运行
...
最后利用豆包的力量给后端凑了个前端，然后也上传到服务器了http://47.109.204.208:8080，上传过程出乎意料简单，倒是花了很多时间把需求跟豆包说清楚，要不然这代码根本跑不了，到最后还有商品更新功能没问题但是完成提示报错的问题，不知道怎么改了，而且很多地方按键点下去有功能没提示的。丑是丑了点，但是我真怕这个豆包一改又出bug
<a href="https://imgchr.com/i/pVLYhGV"><img src="https://s21.ax1x.com/2025/10/18/pVLYhGV.png" alt="pVLYhGV.png" border="0" /></a>
这个文件中的后端项目是包括前端的在/resources/static/。现在已经是18号晚上了，明天最后提交一下，希望加入焦糖捏。
***